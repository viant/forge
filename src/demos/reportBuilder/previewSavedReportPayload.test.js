...
function resolvePrimaryBuilderBlock(savedReportPayload = null) {
  return savedReportPayload?.reportDocument?.blocks
    ?.find((block) => block?.id === "primaryBuilder" && block?.kind === "reportBuilderBlock") || null;
}

function resolvePrimaryBuilderState(savedReportPayload = null) {
  return resolvePrimaryBuilderBlock(savedReportPayload)?.state || null;
}
...
async function buildReopenedRuntimeDrillScenario({
  ...
  const allRuntimeBlocks = resolveReportRuntimeBlocks(basePreview.reportSpec, basePreview.reportFill);
...
assert.equal(resolvePrimaryBuilderState(inventoryLadderRecord.savedReportPayload)?.viewMode, "table");
assert.equal(resolvePrimaryBuilderState(inventoryLadderRecord.savedReportPayload)?.chartSpec?.xField, "channelV2");
assert.equal(resolvePrimaryBuilderState(inventoryLadderRecord.savedReportPayload)?.reportDocumentTemplateId, "forecast_inventory_brief");
assert.equal(resolvePrimaryBuilderState(inventoryLadderRecord.savedReportPayload)?.reportDocumentTemplateLabel, "Forecast Inventory Brief");
assert.equal(resolvePrimaryBuilderState(inventoryLadderRecord.savedReportPayload)?.activeTablePreset?.title, "Inventory Ladder");
assert.equal(resolvePrimaryBuilderState(inventoryLadderRecord.savedReportPayload)?.lastTablePreset?.title, "Inventory Ladder");
...
assert.equal(resolvePrimaryBuilderState(locationLadderRecord.savedReportPayload)?.viewMode, "table");
assert.equal(resolvePrimaryBuilderState(locationLadderRecord.savedReportPayload)?.chartSpec?.xField, "country");
assert.equal(resolvePrimaryBuilderState(locationLadderRecord.savedReportPayload)?.reportDocumentTemplateId, "forecast_location_brief");
assert.equal(resolvePrimaryBuilderState(locationLadderRecord.savedReportPayload)?.reportDocumentTemplateLabel, "Forecast Location Brief");
assert.equal(resolvePrimaryBuilderState(locationLadderRecord.savedReportPayload)?.activeTablePreset?.title, "Location Ladder");
assert.equal(resolvePrimaryBuilderState(locationLadderRecord.savedReportPayload)?.lastTablePreset?.title, "Location Ladder");
assert.deepEqual(
...
assert.equal(resolvePrimaryBuilderState(landscapeInventoryLadderRecord.savedReportPayload)?.reportDocumentTemplateId, "forecast_inventory_brief");
assert.deepEqual(
...
assert.equal(resolvePrimaryBuilderState(landscapeLocationRecord.savedReportPayload)?.reportDocumentTemplateId, "forecast_location_brief");
...
assert.deepEqual(resolvePrimaryBuilderState(marketEfficiencyRecord.savedReportPayload)?.selectedMeasures, ["avails"]);
assert.equal(resolvePrimaryBuilderState(marketEfficiencyRecord.savedReportPayload)?.reportDocumentTemplateId, "market_efficiency_brief");
assert.equal(resolvePrimaryBuilderState(marketEfficiencyRecord.savedReportPayload)?.reportDocumentTemplateLabel, "Market Efficiency Brief");
...
assert.equal(marketEfficiencyRecord.exportRequest.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Reach Rate: 40.8%"), true);
...
assert.equal(reopenedMarketEfficiencyPreview.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Reach Rate: 40.8%"), true);
  ...
  refinements: [
    {
      ...
      values: ["US"],
      sourceBlockId: "reachRateTrend",
      label: "Drill to Region: Market = US",
    },
  ],
});
...
assert.equal(marketEfficiencyRuntimeDrill.preview.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Drill to Region: Market = US"), true);
...
const marketEfficiencyAfterChartBlocks = resolveReportRuntimeBlocks(
...
assert.deepEqual(
  ...
  [
    { valueKey: "region", displayValueKey: "region", runtimeFilterable: true },
...
assert.equal(marketEfficiencyMixedPreview.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Drill to Region: Market = US"), true);
...
assert.equal(marketEfficiencyAfterUndo.preview.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Drill to Region: Market = US"), true);
...
assert.equal(marketEfficiencyAfterRemove.preview.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Drill to Region: Market = US"), false);
...
assert.equal(marketEfficiencyAfterClear.preview.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Drill to Region: Market = US"), false);
...
assert.equal(resolvePrimaryBuilderState(inventoryRecord.savedReportPayload)?.viewMode, "chart");
assert.deepEqual(resolvePrimaryBuilderState(inventoryRecord.savedReportPayload)?.selectedMeasures, ["avails"]);
assert.deepEqual(resolvePrimaryBuilderState(inventoryRecord.savedReportPayload)?.selectedDimensions, ["channelV2"]);
assert.equal(resolvePrimaryBuilderState(inventoryRecord.savedReportPayload)?.chartSpec.xField, "channelV2");
assert.equal(resolvePrimaryBuilderState(inventoryRecord.savedReportPayload)?.chartSpec.title, "Inventory · Top Channels");
assert.equal(resolvePrimaryBuilderState(inventoryRecord.savedReportPayload)?.reportDocumentTemplateId, "forecast_inventory_brief");
assert.equal(resolvePrimaryBuilderState(inventoryRecord.savedReportPayload)?.reportDocumentTemplateLabel, "Forecast Inventory Brief");
...
assert.deepEqual(resolvePrimaryBuilderState(locationRecord.savedReportPayload)?.selectedMeasures, ["avails"]);
assert.deepEqual(resolvePrimaryBuilderState(locationRecord.savedReportPayload)?.selectedDimensions, ["country"]);
assert.equal(resolvePrimaryBuilderState(locationRecord.savedReportPayload)?.chartSpec.xField, "country");
assert.equal(resolvePrimaryBuilderState(locationRecord.savedReportPayload)?.chartSpec.title, "Locations · Top Markets");
assert.equal(resolvePrimaryBuilderState(locationRecord.savedReportPayload)?.reportDocumentTemplateId, "forecast_location_brief");
assert.equal(resolvePrimaryBuilderState(locationRecord.savedReportPayload)?.reportDocumentTemplateLabel, "Forecast Location Brief");
...
const mixedLocationAfterChartBlocks = resolveReportRuntimeBlocks(
