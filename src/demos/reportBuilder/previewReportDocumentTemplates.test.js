...
assert.deepEqual(
  ...
  [
    "market_brief",
    "market_efficiency_brief",
    "market_operations_brief",
    "capacity_inventory_brief",
    "capacity_location_brief",
    "forecast_inventory_brief",
    "forecast_location_brief",
    "forecast_country_dashboard_brief",
  ],
);
...
const marketOperationsBase = buildTemplateRuntime("market_operations_brief");
assert.equal(marketOperationsBase.instantiated.nextState.reportDocumentTitle, "Market Operations Brief");
assert.deepEqual(marketOperationsBase.instantiated.nextState.reportDocumentLayout, {
  type: "stack",
  items: [
    { blockId: "scopeFilters" },
    { blockId: "sectionTabs" },
    ...
    { blockId: "contextPanel" },
    { blockId: "statusCallout" },
    { blockId: "marketSnapshot" },
  ],
});
assert.deepEqual(
  marketOperationsBase.model.reportSpec.layoutIntent.items,
  [
...
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "sectionTabs")?.kind, "tabGroupBlock");
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "overviewSection")?.kind, "sectionBlock");
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "summaryComposite")?.childBlockIds?.length, 3);
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "contextPanel")?.kind, "infoPanelBlock");
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "statusCallout")?.kind, "calloutBlock");
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "marketSnapshot")?.kind, "collectionBlock");
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "deliveryFlow")?.kind, "stepperBlock");
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "activationBoard")?.kind, "kanbanBlock");
assert.equal(marketOperationsBase.model.reportSpec.blocks.find((block) => block.id === "milestoneTimeline")?.kind, "timelineBlock");
assert.equal(marketOperationsBase.preview.reportFill.blocks.find((block) => block.id === "marketSnapshot")?.content?.items?.length, 2);
assert.equal(marketOperationsBase.preview.reportFill.blocks.find((block) => block.id === "deliveryFlow")?.content?.steps?.length, 3);
assert.equal(marketOperationsBase.preview.reportFill.blocks.find((block) => block.id === "activationBoard")?.content?.columns?.length, 2);
assert.equal(marketOperationsBase.preview.reportFill.blocks.find((block) => block.id === "milestoneTimeline")?.content?.events?.length, 2);
assert.equal(marketOperationsBase.preview.reportPrint.bookmarks.some((bookmark) => bookmark.id === "bookmark.summaryComposite"), true);
assert.equal(marketOperationsBase.preview.reportPrint.bookmarks.some((bookmark) => bookmark.id === "bookmark.deliveryFlow"), true);
assert.equal(marketOperationsBase.preview.reportPrint.bookmarks.some((bookmark) => bookmark.id === "bookmark.contextPanel"), true);
assert.equal(marketOperationsBase.preview.reportPrint.pages.flatMap((page) => page.elements || []).some((element) => element.kind === "text" && element.text === "Activation Board"), true);
...
const countryDashboardTemplate = templates.find((entry) => entry.id === "forecast_country_dashboard_brief");
assert.ok(countryDashboardTemplate, "Forecast country dashboard template should exist.");
const countryDashboard = instantiateReportBuilderDocumentTemplate(config, countryDashboardTemplate);
assert.equal(countryDashboard.valid, true, "Forecast country dashboard template should instantiate cleanly.");
assert.deepEqual(countryDashboard.nextState.reportDocumentLayout, {
  type: "stack",
  items: [
    { blockId: "scopeFilters" },
    { blockId: "primaryBuilder" },
...
const countryDashboardModel = buildReportBuilderRuntimePreviewModel({
  container,
  config,
  state: countryDashboard.nextState,
});
assert.ok(countryDashboardModel?.reportSpec, "Forecast country dashboard should build a report spec.");
assert.deepEqual(
  countryDashboardModel.reportSpec.datasets.map((dataset) => dataset.id),
  ["primary", "forecast_country_snapshot"],
);
assert.equal(
  countryDashboardModel.reportSpec.blocks.some((block) => block?.datasetRef === "forecast_country_snapshot"),
  true,
);

console.log("previewReportDocumentTemplates ✓ forecasting authored templates preserve layout sizing, align with 3-step drill ladders, and include mixed dataset starters");
