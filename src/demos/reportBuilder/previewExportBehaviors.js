...
export function normalizePreviewExportBehavior(behavior = {}) {
  ...
  const normalizedMatch = {
    ...(normalizeString(match.phase).toLowerCase() ? { phase: normalizeString(match.phase).toLowerCase() } : {}),
    ...(normalizeString(match.source) ? { source: normalizeString(match.source) } : {}),
    ...(normalizeString(match.format).toLowerCase() ? { format: normalizeString(match.format).toLowerCase() } : {}),
    ...(normalizeString(match.artifactRef) ? { artifactRef: normalizeString(match.artifactRef) } : {}),
    ...(normalizeString(match.title) ? { title: normalizeString(match.title) } : {}),
    ...(normalizeString(match.jobId) ? { jobId: normalizeString(match.jobId) } : {}),
    ...(normalizeString(match.artifactId) ? { artifactId: normalizeString(match.artifactId) } : {}),
  };
...
function matchesPreviewExportBehavior(behavior = {}, {
  phase = "",
  source = "",
  format = "",
  artifactRef = "",
  ...
  if (normalizeString(match.source) && normalizeString(match.source).toLowerCase() !== normalizeString(source).toLowerCase()) {
    return false;
  }
...
export function consumePreviewExportBehavior(metrics = {}, query = {}) {
  ...
  const index = behaviors.findIndex((behavior) => matchesPreviewExportBehavior(behavior, query));
  if (index === -1) {
    return null;
  }
