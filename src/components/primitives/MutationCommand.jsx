...
export default function MutationCommand({command, context, extras = {}, disabled = false, children, onSettled}) {
  ...
  const blockedExplanation = blocked
    ? (!valid ? command?.invalidMessage || command?.tooltip : command?.tooltip)
    : '';
  ...
  return (
    <span className="forge-mutation-command" title={blockedExplanation || undefined} tabIndex={blockedExplanation ? 0 : undefined} aria-label={blockedExplanation ? `${command?.label || 'Action'}. ${blockedExplanation}` : undefined}>
