...
function resolveReportPrintTonePalette(tone = "") {
  ...
  switch (normalized) {
    ...
    case "danger":
    case "critical":
      return { backgroundColor: "#fdecea", borderColor: "#db3737", textColor: "#a82a2a" };
    case "warning":
      return { backgroundColor: "#fff5d6", borderColor: "#f5c542", textColor: "#8a5d00" };
    case "info":
      return { backgroundColor: "#ebf1f5", borderColor: "#ced9e0", textColor: "#30404d" };
    case "reviewed":
      return { backgroundColor: "#eff8ff", borderColor: "#175cd3", textColor: "#175cd3" };
    case "certified":
      return { backgroundColor: "#eef8f0", borderColor: "#0f9960", textColor: "#0a6640" };
    default:
      return { backgroundColor: "#f2f4f7", borderColor: "#d0d5dd", textColor: "#344054" };
  }
}
...
function renderReportPrintTableRow(state = {}, block = {}, columns = [], row = {}, rowPosition = 0, rowSequence = 0, columnWidth = 0, dataBarRanges = new Map()) {
  ...
  columns.forEach((column, columnIndex) => {
    ...
    if (normalizeString(visualState?.kind) === "dataBar" && typeof visualState?.value === "number") {
      ...
      pushReportPrintElement(state, buildReportPrintRectElement({
        id: `${normalizeString(block?.id || "table")}__${rowKey}__${columnKey}__bar_track`,
        kind: "rect",
        box: {
          x: cellInnerX,
          y: cellInnerY,
      ...
      pushReportPrintElement(state, buildReportPrintTableCellDataBarElement({
        id: `${normalizeString(block?.id || "table")}__${rowKey}__${columnKey}__bar`,
        kind: "tableCellDataBar",
        box: {
          x: cellInnerX,
          y: cellInnerY,
          width: fillWidth > 0 ? Math.min(cellInnerWidth, Math.max(6, fillWidth)) : 0,
          height: cellInnerHeight,
        },
        rowKey,
        columnKey,
      ...
      const colors = resolveReportPrintTonePalette(visualState?.tone);
      ...
      return;
    } else if (normalizeString(visualState?.kind) === "tone") {
      const colors = resolveReportPrintTonePalette(visualState?.tone);
      pushReportPrintElement(state, buildReportPrintTableCellToneElement({
        id: `${normalizeString(block?.id || "table")}__${rowKey}__${columnKey}__tone`,
        kind: "tableCellTone",
        box: {
          x: cellInnerX,
          y: cellInnerY,
