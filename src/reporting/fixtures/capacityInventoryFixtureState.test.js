...
const fixtureState = buildCapacityInventoryFixtureState();

assert.equal(fixtureState.record.savedReportPayload.reportDocument.id, "capacityInventoryTopChannelsQ3");
assert.equal(fixtureState.savedReportRecord.savedReportPayload.reportDocument.id, "capacityInventoryTopChannelsQ3");
assert.equal(fixtureState.reportPrint.kind, "reportPrint");
assert.equal(fixtureState.pdfReportPrint.kind, "reportPrint");
assert.equal(fixtureState.reportPrint.title, "Capacity Inventory Top Channels Q3");
...
assert.equal(fixtureState.getReportDocumentResponse.reportRef.reportId, "capacityInventoryTopChannelsQ3");
assert.equal(fixtureState.getReportDocumentResponse.documentVersion, 7);
assert.equal(fixtureState.listReportDocumentsResponse.kind, "listReportDocumentsResponse");
assert.equal(fixtureState.listReportDocumentsResponse.entries[0].reportRef.reportId, "capacityInventoryTopChannelsQ3");
assert.equal(fixtureState.createReportDocumentPayload.kind, "createReportDocumentPayload");
assert.equal(fixtureState.createReportDocumentPayload.reportRef.reportId, "capacityInventoryTopChannelsQ3");
assert.equal(fixtureState.getReportDocumentRequest.kind, "getReportDocumentRequest");
assert.equal(fixtureState.getReportDocumentRequest.reportRef.reportId, "capacityInventoryTopChannelsQ3");
assert.equal(fixtureState.updateReportDocumentPayload.kind, "updateReportDocumentPayload");
assert.equal(fixtureState.updateReportDocumentPayload.expectedVersion, 7);
...
console.log("capacityInventoryFixtureState ✓ exposes the canonical inventory runtime/export fixture set");
