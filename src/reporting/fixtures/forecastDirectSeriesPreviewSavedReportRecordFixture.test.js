...
const {
  ...
  primaryBuilderBlock,
  savedReportPayload,
} = buildForecastDirectSeriesFixtureState();

assert.equal(savedReportPayload.reportDocument.id, "forecastingKpiBlendByDateQ3");
assert.equal(savedReportPayload.reportDocument.title, "Forecasting KPI Blend Q3");
...
assert.equal(exportRequest.reportPrint.title, "Forecasting KPI Blend Q3");
assert.equal(exportRequest.source.reportId, "forecastingKpiBlendByDateQ3");
assert.equal(exportRequest.source.documentVersion, 9);
assert.equal(exportRequest.source.sourceArtifactId, "forecasting_q3_kpi_blend_by_date");
...
console.log("forecastDirectSeriesPreviewSavedReportRecordFixture ✓ direct-series saved record preserves KPI blend print layout and reopen print parity");
