...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-inventory-chart-action-provider-retry.scenario.mjs";
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("inventory-chart-action-provider-retry")),
  true,
);
...
const clickRetryIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("Retry action provider button not found in reopened inventory chart surface."));
...
console.log("report-builder-preview-reopen-report-document-inventory-chart-action-provider-retry-scenario-assets ✓ reopened inventory chart action provider retry restores runtime actions after retry");
