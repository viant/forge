...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-table-detail-error-recovery.scenario.mjs";
...
assert.equal(
  scenario.steps.some((step) => step?.type === "clickSelectorContains" && step?.selector === ".forge-report-builder__runtime-preview .forge-report-runtime-table-panel .forge-dashboard-row-action" && step?.text === "Show channel details" && step?.index === 1),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("table-detail-error-recovery")),
  true,
);
...
assert.equal(failureClickIndex < failureCheckIndex, true);
...
console.log("report-builder-preview-reopen-report-document-table-detail-error-recovery-scenario-assets ✓ reopened table detail failure and recovery flow preserves runtime resolution");
