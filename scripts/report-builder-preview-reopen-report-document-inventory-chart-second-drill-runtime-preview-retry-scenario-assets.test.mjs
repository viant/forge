...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-inventory-chart-second-drill-runtime-preview-retry.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("__inventorySecondDrillBaseline")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes("__inventoryLeafDrillRetryBaseline")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("inventory-chart-second-drill-runtime-preview-retry")),
  true,
);

const reopenIndex = findStepIndex((step) => step?.type === "clickRole" && step?.name === "Reopen in builder");
const firstDrillBaselineIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("__inventorySecondDrillBaseline"));
...
const retryBaselineIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("__inventoryLeafDrillRetryBaseline"));
...
const secondDrillConfirmIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes("Drill to Site Type = Acme Media") && scenario.steps.indexOf(step) > retryRecoverIndex);
...
console.log("report-builder-preview-reopen-report-document-inventory-chart-second-drill-runtime-preview-retry-scenario-assets ✓ reopened inventory second drill retry restores preview and preserves nested drill state");
