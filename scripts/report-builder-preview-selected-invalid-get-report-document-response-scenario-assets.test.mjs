...
import scenario from "../tests/report-builder-preview-semantic-selected-invalid-get-report-document-response.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("patchSeededSavedReportPayload") && expression.includes("capacityQ3") && expression.includes("documentBlockValueFieldUnavailable")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("Headline KPI references unavailable KPI value field 'avails'.")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("selected-invalid-get-report-document-response")),
  true,
);

const patchInvalidIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("patchSeededSavedReportPayload"));
...
console.log("report-builder-preview-selected-invalid-get-report-document-response-scenario-assets ✓ selected invalid get response preserves compile warnings through local response open and reopen");
