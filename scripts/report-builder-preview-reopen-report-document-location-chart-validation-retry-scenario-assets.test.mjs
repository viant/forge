...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-location-chart-validation-retry.scenario.mjs";
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("location-chart-validation-retry")),
  true,
);
...
console.log("report-builder-preview-reopen-report-document-location-chart-validation-retry-scenario-assets ✓ reopened location chart validation retry recovers after semantic provider validation retry");
