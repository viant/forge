...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-inventory-chart-toggle-cache.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("__reopenedInventoryChartToggleCacheBaseline") && expression.includes('entry.type === "chartQuery"')),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes("Inventory · Top Channels") && expression.includes("binding?.mode === 'semantic'")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("inventory-chart-toggle-cache")),
  true,
);

const reopenIndex = findStepIndex((step) => step?.type === "clickRole" && step?.name === "Reopen in builder");
const resetBaselineIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("__reopenedInventoryChartToggleCacheBaseline"));
...
const finalStableIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes('preview?.["__reopenedInventoryChartToggleCacheBaseline"]'));
...
console.log("report-builder-preview-reopen-report-document-inventory-chart-toggle-cache-scenario-assets ✓ reopened inventory chart toggle-cache preserves chart/table runtime stability");
