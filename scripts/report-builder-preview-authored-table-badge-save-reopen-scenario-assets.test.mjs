...
import scenario from "../tests/report-builder-preview-semantic-authored-table-badge-save-reopen.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("applyAuthoredDocumentBlock API not available.") && expression.includes("comparisonTable") && expression.includes("kind: 'badge'") && expression.includes("value: 'Display'") && expression.includes("value: 'CTV'")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.filter((expression) => expression.includes("Channel badge")).length >= 2,
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes(".forge-dashboard-table-cell-visual--badge") && expression.includes("DISPLAY") && expression.includes("CTV")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("\"kind\": \"getReportDocumentResponse\"") && expression.includes("\"title\": \"Comparison Table\"") && expression.includes("\"kind\": \"badge\"") && expression.includes("\"tone\": \"info\"") && expression.includes("\"value\": \"Display\"") && expression.includes("\"tone\": \"success\"") && expression.includes("\"value\": \"CTV\"")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes(".forge-report-runtime-table-panel") && expression.includes("Comparison Table") && expression.includes("Available Impressions") && expression.includes(".forge-dashboard-table-cell-visual--badge") && expression.includes("DISPLAY") && expression.includes("CTV")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("\"kind\": \"badge\"")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("\"tone\": \"success\"")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("authored-table-badge-save-reopen")),
  true,
);
...
const badgeSummaryIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("Channel badge"));
const badgeVisualIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes(".forge-dashboard-table-cell-visual--badge") && String(step.expression || "").includes("DISPLAY") && String(step.expression || "").includes("CTV"));
...
const inspectGetPayloadIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("\"kind\": \"getReportDocumentResponse\"") && String(step.expression || "").includes("\"kind\": \"badge\"") && String(step.expression || "").includes("\"value\": \"CTV\""));
...
assert.notEqual(badgeSummaryIndex, -1);
assert.notEqual(badgeVisualIndex, -1);
...
assert.equal(addTableCardIndex < badgeSummaryIndex, true);
assert.equal(badgeSummaryIndex < badgeVisualIndex, true);
assert.equal(badgeVisualIndex < draftIndex, true);
...
const reopenedBadgeSummaryStep = scenario.steps
  .slice(reopenIndex + 1)
  .find((step) => step?.type === "waitForEval" && String(step.expression || "").includes("Channel badge"));

assert.notEqual(reopenedBadgeSummaryStep, undefined);

console.log("report-builder-preview-authored-table-badge-save-reopen-scenario-assets ✓ authored badge table survives save/get/reopen with inspected get payload and restored runtime preview");
