...
import scenario from "../tests/report-builder-preview-semantic-authored-chart-save-reopen-detail.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("Show channel details") && expression.includes("Selected value:") && expression.includes("Display")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes(".forge-report-runtime-host-intent") && expression.includes("target://example/performance/channel-detail") && expression.includes("Display")),
  true,
);
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("authored-chart-save-reopen-detail")),
  true,
);
...
const detailActionIndex = findStepIndex((step) => step?.type === "eval" && String(step.expression || "").includes("Capacity Trend Block detail action not found."));
const hostIntentIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes(".forge-report-runtime-host-intent") && String(step.expression || "").includes("target://example/performance/channel-detail"));
...
console.log("report-builder-preview-authored-chart-save-reopen-detail-scenario-assets ✓ authored chart detail action survives save/get/reopen after post-save removal");
