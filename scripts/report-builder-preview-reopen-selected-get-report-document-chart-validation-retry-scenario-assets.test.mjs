...
import scenario from "../tests/report-builder-preview-semantic-reopen-selected-get-report-document-chart-validation-retry.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("getHydratedReportDocumentSession") && expression.includes("capacityTrendQ3") && expression.includes("documentVersion === 6")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("reopen-selected-get-report-document-chart-validation-retry")),
  true,
);
