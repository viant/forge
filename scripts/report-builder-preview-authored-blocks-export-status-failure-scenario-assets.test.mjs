...
import scenario from "../tests/report-builder-preview-semantic-authored-blocks-export-status-failure.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("demo-export-job-1") && expression.includes("failed") && expression.includes("Semantic Binding") && expression.includes("Model Ad Delivery") && expression.includes("Entity Line Delivery") && expression.includes("Reach KPI") && expression.includes("Inventory Note")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("authored-blocks-export-status-failure")),
  true,
);
...
console.log("report-builder-preview-authored-blocks-export-status-failure-scenario-assets ✓ multi-block semantic saved-payload export status failures stay explicit while semantic context remains visible");
