...
assert.equal(
  expressions.some((expression) => expression.includes("JSON.stringify(signature) ===") && expression.includes(exactSignatureLiteral)),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("window.__artifactDownloadCapture?.payloadReady") && expression.includes("JSON.stringify(signature) ===") && expression.includes(exactSignatureLiteral)),
  true,
);
assert.equal(
...
const geometrySummaryIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes("JSON.stringify(signature) ===") && String(step?.expression || "").includes(exactSignatureLiteral));
...
const geometryDownloadedIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes("window.__artifactDownloadCapture?.payloadReady") && String(step?.expression || "").includes("JSON.stringify(signature) ===") && String(step?.expression || "").includes(exactSignatureLiteral));
