...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-chart-validation-retry.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("Validating the semantic selection against the provider.") || expression.includes("Avails by Date and Channel")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("chart-validation-retry")),
  true,
);
...
console.log("report-builder-preview-reopen-report-document-chart-validation-retry-scenario-assets ✓ reopened chart validation retry recovers after semantic provider validation retry");
