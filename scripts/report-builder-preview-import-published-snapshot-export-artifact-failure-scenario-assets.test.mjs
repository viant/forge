...
import scenario from "../tests/report-builder-preview-semantic-import-published-snapshot-export-artifact-failure.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("demo-export-artifact-1") && expression.includes("succeeded") && expression.includes("publishedSnapshot") && expression.includes("reportBuilder.publishedSnapshot://published_snapshot_capacityTrendQ3") && expression.includes("Semantic Binding") && expression.includes("Model Ad Delivery") && expression.includes("Entity Line Delivery")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("import-published-snapshot-export-artifact-failure")),
  true,
);
...
console.log("report-builder-preview-import-published-snapshot-export-artifact-failure-scenario-assets ✓ imported published-snapshot artifact export artifact failures stay explicit while semantic context remains visible");
