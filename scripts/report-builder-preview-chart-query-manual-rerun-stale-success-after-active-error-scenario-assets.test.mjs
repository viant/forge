...
import scenario from "../tests/report-builder-preview-semantic-chart-query-manual-rerun-stale-success-after-active-error.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("run button not found") && expression.includes("manual rerun active error setup")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("__chartManualRerunActiveErrorPreStaleCounts") && expression.includes("current.errorCount === 1") && expression.includes("current.successCount === 0") && expression.includes("text.includes('Active chart query failed.')") && expression.includes("!text.includes('Stale chart query failed.')")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("__chartManualRerunActiveErrorFinalCounts") && expression.includes("current.errorCount === 1") && expression.includes("current.successCount === 1") && expression.includes("text.includes('Active chart query failed.')")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("chart-query-manual-rerun-stale-success-after-active-error")),
  true,
);
...
const activeErrorVisibleIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("__chartManualRerunActiveErrorPreStaleCounts"));
...
console.log("report-builder-preview-chart-query-manual-rerun-stale-success-after-active-error-scenario-assets ✓ stale delayed rerun successes never own the surface over the active rerun error state");
