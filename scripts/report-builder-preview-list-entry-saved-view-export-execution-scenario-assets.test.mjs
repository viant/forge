...
import scenario from "../tests/report-builder-preview-semantic-list-entry-saved-view-export-execution.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("demo-export-job-1") && expression.includes("queued") && expression.includes("imported saved-view") && expression.includes("saved-view artifact") && expression.includes("Semantic Binding") && expression.includes("Model Ad Delivery") && expression.includes("Entity Line Delivery")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("window.__artifactDownloadCapture.mimeType === 'application/pdf'") && expression.includes("%PDF-demo Capacity Trend Q3 Saved View")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("list-entry-saved-view-export-execution")),
  true,
);
...
console.log("report-builder-preview-list-entry-saved-view-export-execution-scenario-assets ✓ selected saved-view list-entry export execution preserves request, status, artifact, and shared-artifact semantic context end to end");
