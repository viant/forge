...
import scenario from "../tests/report-builder-preview-semantic-selected-entry-ambiguous-local-backing.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("replaceSeededSavedReportPayloads") && expression.includes("capacityShared") && expression.includes("reportBuilder.savedView") && expression.includes("reportBuilder.publishedSnapshot")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes("replacePreparedListReportDocumentsResponse") && expression.includes("title: \"Capacity Shared\"")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("ambiguous local backing")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("Get ReportDocument request summary") && expression.includes("ambiguous local backing")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("selected-entry-ambiguous-local-backing")),
  true,
);
...
const selectedEntryIndex = findStepIndex((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("Selected entry: Capacity Shared"));
const blockerChipIndex = findStepIndex((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("ambiguous local backing"));
...
console.log("report-builder-preview-selected-entry-ambiguous-local-backing-scenario-assets ✓ ambiguous source-less selected catalog entries surface explicit export and request blockers in the live preview flow");
