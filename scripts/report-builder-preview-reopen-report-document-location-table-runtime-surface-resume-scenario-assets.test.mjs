...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-location-table-runtime-surface-resume.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("replaceStandaloneRuntimeInteraction") && expression.includes("Drill to Region = US") && expression.includes("nextFieldRef: 'region'")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes("advanceStandaloneRuntimeInteraction") && expression.includes("Drill to Metro Area = West") && expression.includes("nextFieldRef: 'metrocode'")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes("activeLabels.length === 1") && expression.includes("labels.includes('Region')")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes("labels.includes('Metro Area')") && expression.includes("activeLabels.length === 1")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("Market • Deprecated")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("semanticGovernance")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("reopen-report-document-location-table-runtime-surface-resume")),
  true,
);
...
const regionResumeIndex = scenario.steps.findIndex((step, index) => index > reloadIndex && step?.type === "waitForEval" && String(step?.expression || "").includes("labels.includes('Region')") && String(step?.expression || "").includes("activeLabels.length === 1"));
...
const finalHeaderIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes("labels.includes('Metro Area')") && String(step?.expression || "").includes("activeLabels.length === 1"));
...
assert.equal(reloadIndex < regionResumeIndex, true);
assert.equal(regionResumeIndex < advanceInteractionIndex, true);
assert.equal(advanceInteractionIndex < finalHeaderIndex, true);

console.log("report-builder-preview-reopen-report-document-location-table-runtime-surface-resume-scenario-assets ✓ reopened location table runtime surface resumes standalone interaction state across reload and second drill");
