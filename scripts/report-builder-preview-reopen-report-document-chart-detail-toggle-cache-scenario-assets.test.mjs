...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-chart-detail-toggle-cache.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("__reopenedChartDetailToggleCacheBaseline") && expression.includes('entry.type === "chartQuery"')),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes('text.includes("target://example/performance/channel-detail")') && expression.includes('text.includes("Display")')),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("chart-detail-toggle-cache")),
  true,
);

const reopenIndex = findStepIndex((step) => step?.type === "clickRole" && step?.name === "Reopen in builder");
const resetBaselineIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("__reopenedChartDetailToggleCacheBaseline"));
...
const resolvedDisplayIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes('text.includes("target://example/performance/channel-detail")') && String(step?.expression || "").includes('text.includes("Display")'));
...
const finalStableIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes('preview?.["__reopenedChartDetailToggleCacheBaseline"]') && scenario.steps.indexOf(step) > toggleChartIndex);

assert.notEqual(reopenIndex, -1);
assert.notEqual(resetBaselineIndex, -1);
assert.notEqual(displayLegendIndex, -1);
assert.notEqual(resolvedDisplayIndex, -1);
...
assert.equal(displayLegendIndex < resolvedDisplayIndex, true);
assert.equal(resolvedDisplayIndex < toggleTableIndex, true);
assert.equal(toggleTableIndex < toggleChartIndex, true);
assert.equal(toggleChartIndex < finalStableIndex, true);

console.log("report-builder-preview-reopen-report-document-chart-detail-toggle-cache-scenario-assets ✓ reopened chart detail toggle-cache preserves runtime resolution and query stability");
