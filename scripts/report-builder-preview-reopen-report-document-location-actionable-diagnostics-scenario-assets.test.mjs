...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-location-actionable-diagnostics.scenario.mjs";
...
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("Primary Table references unavailable table column")),
  true,
);
assert.equal(
