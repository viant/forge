...
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForEval" && String(step.expression || "").includes("forge-report-builder__result-header h3") && String(step.expression || "").includes("Delivery Grid")),
  true,
);
assert.equal(
...
const presetActiveIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("forge-report-builder__result-header h3") && String(step.expression || "").includes("Delivery Grid"));
