...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-inventory-chart-drill-runtime-preview-retry.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("__inventoryDrillRetryBaseline") && expression.includes("runtimePreview")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("inventory-chart-drill-runtime-preview-retry")),
  true,
);
...
const retryBaselineIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes("__inventoryDrillRetryBaseline"));
...
console.log("report-builder-preview-reopen-report-document-inventory-chart-drill-runtime-preview-retry-scenario-assets ✓ reopened inventory chart drill retry restores drill runtime after rerun");
