...
import scenario from "../tests/report-builder-preview-semantic-list-entry-saved-view-export-status-failure.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("demo-export-job-1") && expression.includes("failed") && expression.includes("imported saved-view") && expression.includes("saved-view artifact") && expression.includes("Semantic Binding") && expression.includes("Model Ad Delivery") && expression.includes("Entity Line Delivery")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("list-entry-saved-view-export-status-failure")),
  true,
);
...
console.log("report-builder-preview-list-entry-saved-view-export-status-failure-scenario-assets ✓ selected saved-view list-entry export status failures stay explicit while shared-artifact semantic context remains visible");
