...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-location-chart-toggle-cache.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("__reopenedLocationChartToggleCacheBaseline") && expression.includes('entry.type === "chartQuery"')),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes("Locations · Top Markets") && expression.includes("binding?.mode === 'semantic'")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("location-chart-toggle-cache")),
  true,
);

const reopenIndex = findStepIndex((step) => step?.type === "clickRole" && step?.name === "Reopen in builder");
const resetBaselineIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("__reopenedLocationChartToggleCacheBaseline"));
...
const finalStableIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes('preview?.["__reopenedLocationChartToggleCacheBaseline"]'));
...
console.log("report-builder-preview-reopen-report-document-location-chart-toggle-cache-scenario-assets ✓ reopened location chart toggle-cache preserves chart/table runtime stability");
