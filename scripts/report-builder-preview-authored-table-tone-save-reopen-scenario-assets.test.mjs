...
import scenario from "../tests/report-builder-preview-semantic-authored-table-tone-save-reopen.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("applyAuthoredDocumentBlock API not available.") && expression.includes("comparisonTable") && expression.includes("kind: 'tone'") && expression.includes("value: 'Display'") && expression.includes("value: 'CTV'")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.filter((expression) => expression.includes("Channel tone")).length >= 2,
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes(".forge-dashboard-table-cell-visual--tone") && expression.includes("entry.text === 'Display'") && expression.includes("entry.tone === 'info'") && expression.includes("entry.text === 'CTV'") && expression.includes("entry.tone === 'warning'")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("\"kind\": \"getReportDocumentResponse\"") && expression.includes("\"title\": \"Comparison Table\"") && expression.includes("\"kind\": \"tone\"") && expression.includes("\"tone\": \"info\"") && expression.includes("\"value\": \"Display\"") && expression.includes("\"tone\": \"warning\"") && expression.includes("\"value\": \"CTV\"")),
  true,
);
assert.equal(
  expressions.some((expression) => expression.includes(".forge-report-runtime-table-panel") && expression.includes("Comparison Table") && expression.includes("Available Impressions") && expression.includes(".forge-dashboard-table-cell-visual--tone") && expression.includes("entry.text === 'Display'") && expression.includes("entry.tone === 'info'") && expression.includes("entry.text === 'CTV'") && expression.includes("entry.tone === 'warning'")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("\"kind\": \"tone\"")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("\"tone\": \"info\"")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("\"value\": \"Display\"")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("\"tone\": \"warning\"")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "waitForDomContains" && String(step.text || "").includes("\"value\": \"CTV\"")),
  true,
);
assert.equal(
...
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("authored-table-tone-save-reopen")),
  true,
);
...
const toneSummaryIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("Channel tone"));
const toneVisualIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes(".forge-dashboard-table-cell-visual--tone") && String(step.expression || "").includes("entry.tone === 'warning'"));
...
const inspectGetPayloadIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("\"kind\": \"getReportDocumentResponse\"") && String(step.expression || "").includes("\"kind\": \"tone\"") && String(step.expression || "").includes("\"tone\": \"warning\""));
...
const runtimeTableIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes(".forge-report-runtime-table-panel") && String(step.expression || "").includes("Comparison Table") && String(step.expression || "").includes(".forge-dashboard-table-cell-visual--tone") && String(step.expression || "").includes("entry.text === 'Display'") && String(step.expression || "").includes("entry.tone === 'info'") && String(step.expression || "").includes("entry.text === 'CTV'") && String(step.expression || "").includes("entry.tone === 'warning'"));

assert.notEqual(addTableIndex, -1);
assert.notEqual(addTableCardIndex, -1);
assert.notEqual(toneSummaryIndex, -1);
assert.notEqual(toneVisualIndex, -1);
...
assert.equal(addTableCardIndex < toneSummaryIndex, true);
assert.equal(toneSummaryIndex < toneVisualIndex, true);
assert.equal(toneVisualIndex < draftIndex, true);
...
const reopenedToneSummaryStep = scenario.steps
  .slice(reopenIndex + 1)
  .find((step) => step?.type === "waitForEval" && String(step.expression || "").includes("Channel tone"));

assert.notEqual(reopenedToneSummaryStep, undefined);

console.log("report-builder-preview-authored-table-tone-save-reopen-scenario-assets ✓ authored tone table survives save/get/reopen with inspected get payload and restored runtime preview");
