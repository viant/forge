...
import scenario from "../tests/report-builder-preview-semantic-reopen-report-document-location-table-action-provider-retry.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("Failed to load refinement actions for Market.") && expression.includes("Preview runtime action provider failed.")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("target://example/performance/market-detail") && expression.includes("country") && expression.includes("CA")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("location-table-action-provider-retry")),
  true,
);
...
const replaceSuccessIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("detail_market") && String(step?.expression || "").includes("delayMs: 1200"));
const clickRetryIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("Retry action provider button not found in reopened runtime table surface."));
const clickActionIndex = findStepIndex((step) => step?.type === "eval" && String(step?.expression || "").includes("Show market details action not found after provider retry."));
const resolvedIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step?.expression || "").includes("target://example/performance/market-detail") && String(step?.expression || "").includes("CA"));
...
console.log("report-builder-preview-reopen-report-document-location-table-action-provider-retry-scenario-assets ✓ reopened location table action provider retry restores runtime actions after retry");
