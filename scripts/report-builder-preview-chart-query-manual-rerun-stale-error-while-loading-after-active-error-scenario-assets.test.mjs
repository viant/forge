...
import scenario from "../tests/report-builder-preview-semantic-chart-query-manual-rerun-stale-error-while-loading-after-active-error.scenario.mjs";
...
assert.equal(
  expressions.some((expression) => expression.includes("preview.fetchBehaviors.length === 2") && expression.includes("preview.fetchBehaviors[0]?.delayMs === 1200") && expression.includes("preview.fetchBehaviors[1]?.delayMs === 5000") && expression.includes("preview.fetchBehaviors[1]?.error === 'Active chart query failed.'")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("run button not found") && expression.includes("manual rerun dual error setup")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("__chartManualRerunDualErrorPreActiveCounts") && expression.includes("current.errorCount === 1") && expression.includes("current.successCount === 0") && expression.includes("!text.includes('Stale chart query failed.')") && expression.includes("!text.includes('Active chart query failed.')") && expression.includes("!text.includes(\"We couldn't render these results\")")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("__chartManualRerunDualErrorFinalCounts") && expression.includes("current.errorCount === 2") && expression.includes("current.successCount === 0") && expression.includes("text.includes('Active chart query failed.')") && expression.includes("!text.includes('Stale chart query failed.')")),
  true,
);
assert.equal(
...
assert.equal(
  expressions.some((expression) => expression.includes("headerText.includes('Avails by Date and Channel')") && expression.includes("frameText.includes('Active chart query failed.')") && expression.includes("!frameText.includes('Stale chart query failed.')") && expression.includes("!chartWrap")),
  true,
);
assert.equal(
  scenario.steps.some((step) => step?.type === "screenshot" && String(step.file || "").includes("chart-query-manual-rerun-stale-error-while-loading-after-active-error")),
  true,
);
...
const preActiveCountsIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("__chartManualRerunDualErrorPreActiveCounts"));
...
const finalErrorViewIndex = findStepIndex((step) => step?.type === "waitForEval" && String(step.expression || "").includes("frameText.includes('Active chart query failed.')") && String(step.expression || "").includes("!frameText.includes('Stale chart query failed.')"));
...
assert.notEqual(preActiveCountsIndex, -1);
...
assert.equal(rerunStartedIndex < preActiveCountsIndex, true);
assert.equal(preActiveCountsIndex < settledCountsIndex, true);
assert.equal(settledCountsIndex < finalErrorViewIndex, true);

console.log("report-builder-preview-chart-query-manual-rerun-stale-error-while-loading-after-active-error-scenario-assets ✓ stale delayed rerun errors stay hidden while loading, and the eventual active rerun error owns the builder surface");
