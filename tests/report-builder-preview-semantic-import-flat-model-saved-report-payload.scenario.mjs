...
export default {
  ...
  steps: [
    ...
    {
      type: "eval",
      expression: "(() => { const close = Array.from(document.querySelectorAll('button')).find((entry) => ((entry.innerText || entry.textContent || '').trim() === 'Close')); if (close) { close.click(); } const preview = window.__REPORT_BUILDER_PREVIEW__; const input = document.querySelector('input[aria-label=\"Import report file\"]'); if (!preview || typeof preview.getSavedReportPayloads !== 'function' || !input) { throw new Error('import api unavailable'); } const payloads = preview.getSavedReportPayloads(); const target = payloads.find((entry) => (entry?.savedReportPayload?.reportDocument?.id || entry?.reportId) === 'flatAudienceSegmentQ3'); const payload = target?.savedReportPayload || target; if (!payload) { throw new Error('seeded flat saved report payload not found'); } const file = new File([JSON.stringify(payload, null, 2)], 'flat-audience.saved-report-payload.json', { type: 'application/json' }); const transfer = new DataTransfer(); transfer.items.add(file); input.files = transfer.files; input.dispatchEvent(new Event('change', { bubbles: true })); return true; })()",
    },
    {
      type: "waitForDomContains",
      text: "Imported saved report payload Flat Audience Segment Q3.",
      timeoutMs: 60000,
    },
    {
      type: "waitForDomContains",
      text: "Saved report payload: Flat Audience Segment Q3",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; return text.includes('Model Audience Delivery') && text.includes('Entity Audience Line Delivery') && text.includes('Measures Audience Index') && text.includes('Parameters Date Range, Audience Segment'); })()",
      timeoutMs: 60000,
    },
    {
      type: "fillSelector",
    ...
    {
      type: "waitForDomContains",
      text: "Using document version 14.",
      timeoutMs: 60000,
    },
    {
      type: "clickRole",
    ...
    {
      type: "waitForDomContains",
      text: "Selected entry: Flat Audience Segment Q3",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForDomContains",
      text: "Get ReportDocument response: Flat Audience Segment Q3",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-import-flat-model-saved-report-payload.png",
      fullPage: true,
    },
  ],
};
