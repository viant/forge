...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForEval",
      expression: "(() => { const keys = ['reportBuilder.state.demoReportBuilder.demoReportBuilderWindow', 'reportBuilder.state.demoReportBuilder']; return keys.some((key) => { const raw = window.localStorage.getItem(key); if (!raw) { return false; } try { const parsed = JSON.parse(raw); return parsed?.explorationSession?.dirty === false && parsed?.explorationSession?.historyIndex === 0; } catch (_) { return false; } }); })()",
      timeoutMs: 60000,
    },
    {
      type: "clickRole",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-exploration-session.png",
      fullPage: true,
    },
  ],
};
