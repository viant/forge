...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForDomContains",
      text: "Renderer rejected reportPrint for Report Builder Demo.",
      timeoutMs: 60000,
    },
    {
      type: "waitForDomContains",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; return text.includes('demo-export-job-1') && text.includes('failed') && text.includes('Semantic Binding') && text.includes('Model Ad Delivery') && text.includes('Entity Line Delivery') && text.includes('Reach KPI') && text.includes('Inventory Note'); })()",
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-authored-blocks-export-status-failure.png",
      fullPage: true,
    },
  ],
};
