...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForEval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); const panel = Array.from(root?.querySelectorAll('.forge-report-runtime-chart-panel') || []).find((entry) => ((entry.innerText || entry.textContent || '')).includes('Capacity Trend Block')); const text = root?.innerText || root?.textContent || ''; return !!panel && text.includes('Failed to resolve detail target target://example/performance/channel-detail. Detail target resolution failed.') && !text.includes('Resolved detail target'); })()",
      timeoutMs: 60000,
    },
    ...buildClearDetailTargetBehaviorsSteps(),
    {
      type: "eval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); const panel = Array.from(root?.querySelectorAll('.forge-report-runtime-chart-panel') || []).find((entry) => ((entry.innerText || entry.textContent || '')).includes('Capacity Trend Block')); if (!panel) { throw new Error('Capacity Trend Block runtime panel not found.'); } const action = Array.from(panel.querySelectorAll('.forge-chart-legend-action')).find((entry) => ((entry.innerText || entry.textContent || '')).includes('CTV')); if (!action) { throw new Error('Capacity Trend Block CTV legend action not found.'); } action.click(); return true; })()",
    },
    {
      type: "waitForEval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); const panel = Array.from(root?.querySelectorAll('.forge-report-runtime-chart-panel') || []).find((entry) => ((entry.innerText || entry.textContent || '')).includes('Capacity Trend Block')); const text = panel?.innerText || panel?.textContent || ''; return text.includes('Selected value:') && text.includes('CTV') && text.includes('Show channel details'); })()",
      timeoutMs: 60000,
    },
    {
      type: "eval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-authored-chart-save-reopen-detail-error-recovery.png",
      fullPage: true,
    },
  ],
};
