...
export default {
  ...
  steps: [
    ...
    },
    {
      type: "clickRole",
      role: "button",
      name: "Move Inventory Note Copy up",
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const titles = Array.from(document.querySelectorAll('.forge-report-builder__document-block-card')).filter((node) => node && node.offsetParent !== null).map((card) => (card.querySelector('strong')?.innerText || card.querySelector('strong')?.textContent || '').trim()).filter(Boolean); return titles.length >= 2 && titles[0] === 'Inventory Note Copy' && titles[1] === 'Inventory Note'; })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); if (!root) { return false; } const titles = Array.from(root.querySelectorAll('section h3')).map((node) => (node.innerText || node.textContent || '').trim()).filter(Boolean); const bodyMatches = ((root.innerText || root.textContent || '').match(/Author-provided inventory context\\./g) || []).length; return titles.includes('Inventory Note Copy') && titles.includes('Inventory Note') && bodyMatches >= 2; })()",
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-selected-get-report-document-duplicate-response.png",
      fullPage: true,
    },
  ],
};
