...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForDomContains",
      text: "Could not load the preview export artifact for Report Builder Demo.",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; return text.includes('demo-export-artifact-1') && text.includes('succeeded') && text.includes('Semantic Binding') && text.includes('Model Ad Delivery') && text.includes('Entity Line Delivery') && text.includes('Reach KPI') && text.includes('Inventory Note'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-authored-blocks-export-artifact-failure.png",
      fullPage: true,
    },
  ],
};
