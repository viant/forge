...
export default {
  ...
  steps: [
    ...
    {
      type: "eval",
      expression: "(() => { const preview = window.__REPORT_BUILDER_PREVIEW__; if (!preview || typeof preview.applyAuthoredDocumentBlock !== 'function') { throw new Error('applyAuthoredDocumentBlock API not available.'); } const result = preview.applyAuthoredDocumentBlock({ kind: 'chartBlock', seed: { id: 'trendChart', title: 'Capacity Trend Block' } }); return !!result?.valid; })()",
    },
    {
    ...
    {
      type: "waitForEval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); if (!root) { return false; } return Array.from(root.querySelectorAll('.forge-report-runtime-chart-panel')).some((panel) => ((panel.innerText || panel.textContent || '').includes('Capacity Trend Block'))); })()",
      timeoutMs: 60000,
    },
    {
      type: "eval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const panel = document.querySelector('[aria-label=\"Get ReportDocument response summary\"]'); const text = panel?.innerText || panel?.textContent || ''; return text.includes('\"kind\": \"getReportDocumentResponse\"') && text.includes('\"kind\": \"chartBlock\"') && text.includes('\"title\": \"Capacity Trend Block\"') && text.includes('\"blockId\": \"trendChart\"'); })()",
      timeoutMs: 60000,
    },
    {
      type: "eval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-authored-chart-save-reopen.png",
      fullPage: true,
    },
  ],
};
