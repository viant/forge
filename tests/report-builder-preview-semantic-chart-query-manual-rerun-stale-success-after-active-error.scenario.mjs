...
export default {
  ...
  steps: [
    ...
    {
      type: "eval",
      expression: "(() => { const button = document.querySelector('.forge-report-builder__run-button'); if (!button) { throw new Error('run button not found'); } if (button.disabled) { throw new Error('run button disabled during manual rerun active error setup'); } button.click(); return true; })()",
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; const preview = window.__REPORT_BUILDER_PREVIEW__; const current = { errorCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'error').length, successCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'success').length }; const settled = preview.__chartManualRerunActiveErrorPreStaleCounts; const changed = !settled || settled.errorCount !== current.errorCount || settled.successCount !== current.successCount; if (changed) { preview.__chartManualRerunActiveErrorPreStaleCounts = { ...current, lastChangedAt: Date.now() }; return false; } return current.errorCount === 1 && current.successCount === 0 && Date.now() - settled.lastChangedAt >= 350 && text.includes('Active chart query failed.') && !text.includes('Stale chart query failed.') && text.includes(\"We couldn't render these results\"); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; const preview = window.__REPORT_BUILDER_PREVIEW__; const current = { errorCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'error').length, successCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'success').length }; const settled = preview.__chartManualRerunActiveErrorFinalCounts; const changed = !settled || settled.errorCount !== current.errorCount || settled.successCount !== current.successCount; if (changed) { preview.__chartManualRerunActiveErrorFinalCounts = { ...current, lastChangedAt: Date.now() }; return false; } return current.errorCount === 1 && current.successCount === 1 && Date.now() - settled.lastChangedAt >= 1200 && text.includes('Active chart query failed.') && !text.includes('Stale chart query failed.') && text.includes(\"We couldn't render these results\"); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-chart-query-manual-rerun-stale-success-after-active-error.png",
      fullPage: true,
    },
  ],
};
