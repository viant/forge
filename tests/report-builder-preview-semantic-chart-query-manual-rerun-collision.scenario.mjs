...
export default {
  ...
  steps: [
    ...
    {
      type: "eval",
      expression: "(() => { const button = document.querySelector('.forge-report-builder__run-button'); if (!button) { throw new Error('run button not found'); } if (button.disabled) { throw new Error('run button disabled during chart query collision setup'); } button.click(); return true; })()",
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const chartWrap = document.querySelector('.forge-report-builder__chart-wrap'); const chartText = chartWrap?.innerText || chartWrap?.textContent || ''; const preview = window.__REPORT_BUILDER_PREVIEW__; const current = { errorCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'error').length, successCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'success').length }; const settled = preview.__chartManualRerunPreStaleSettledCounts; const changed = !settled || settled.errorCount !== current.errorCount || settled.successCount !== current.successCount; if (changed) { preview.__chartManualRerunPreStaleSettledCounts = { ...current, lastChangedAt: Date.now() }; return false; } return current.errorCount === 0 && current.successCount === 1 && Date.now() - settled.lastChangedAt >= 350 && chartText.includes('Gamma') && chartText.includes('Delta') && !chartText.includes('Stale chart query failed.'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-chart-query-manual-rerun-collision.png",
      fullPage: true,
    },
  ],
};
