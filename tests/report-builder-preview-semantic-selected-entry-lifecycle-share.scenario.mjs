...
const savedViewRecordExpression = `(() => {
  ...
  if (!payload?.reportDocument || !exportRequest) {
    throw new Error('seeded lifecycle saved-view share record source not found');
  }
  return {
...
export default {
  ...
  steps: [
    ...buildInlineArtifactImportSteps({
      ...
      filename: "capacity-trend.lifecycle-share.saved-view.saved-record.json",
    ...
    ...buildInlineArtifactImportSteps({
      ...
      filename: "capacity-trend.lifecycle-share.saved-view.list-response.json",
    ...
    },
    {
      ...
      expression: `(() => {
        ...
        const text = panel?.innerText || panel?.textContent || '';
        return text.includes('"kind": "reportBuilder.savedView"')
          && text.includes('"artifactRef": "reportBuilder.savedView://saved_view_capacityTrendQ3_1"');
      })()`,
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-selected-entry-lifecycle-share.png",
      fullPage: true,
    },
  ],
};
