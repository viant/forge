...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForEval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); const panel = Array.from(root?.querySelectorAll('.forge-report-runtime-chart-panel') || []).find((entry) => ((entry.innerText || entry.textContent || '')).includes('Capacity Trend Block')); const text = panel?.innerText || panel?.textContent || ''; return text.includes('Selected value:') && text.includes('Display') && text.includes('Drill to Market'); })()",
      timeoutMs: 60000,
    },
    {
      type: "eval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); const panel = Array.from(root?.querySelectorAll('.forge-report-runtime-chart-panel') || []).find((entry) => ((entry.innerText || entry.textContent || '')).includes('Capacity Trend Block')); if (!panel) { throw new Error('Capacity Trend Block runtime panel not found.'); } const action = Array.from(panel.querySelectorAll('.forge-report-runtime-chart-action')).find((entry) => ((entry.innerText || entry.textContent || '')).includes('Drill to Market')); if (!action) { throw new Error('Capacity Trend Block drill action not found.'); } action.click(); return true; })()",
    },
    {
      type: "waitForDomContains",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); const chips = Array.from(root?.querySelectorAll('.forge-report-runtime-refinement-chip') || []).map((node) => (node.innerText || node.textContent || '').trim()); if (!chips.some((entry) => entry.includes('Drill to Market = Display'))) { return false; } const panel = root?.querySelector('.forge-report-runtime-table-panel'); const text = panel?.innerText || panel?.textContent || ''; const headers = panel?.querySelectorAll('th') || []; const labels = Array.from(headers).map((entry) => (entry.innerText || entry.textContent || '').trim()); return text.includes('Market') && labels.includes('Market') && !labels.includes('Channel'); })()",
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-authored-chart-save-reopen-drill.png",
      fullPage: true,
    },
  ],
};
