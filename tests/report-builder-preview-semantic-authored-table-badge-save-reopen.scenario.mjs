...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; return text.includes('Channel badge'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
      expression: "(() => { const badges = Array.from(document.querySelectorAll('.forge-dashboard-table-cell-visual--badge')).map((node) => ((node.innerText || node.textContent || '').trim()).toUpperCase()); return badges.includes('DISPLAY') && badges.includes('CTV'); })()",
      timeoutMs: 60000,
    },
    {
      type: "eval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const panels = Array.from(document.querySelectorAll('.forge-report-runtime-table-panel')); return panels.some((panel) => { const text = panel.innerText || panel.textContent || ''; if (!text.includes('Comparison Table') || !text.includes('Available Impressions')) { return false; } const badges = Array.from(panel.querySelectorAll('.forge-dashboard-table-cell-visual--badge')).map((node) => ((node.innerText || node.textContent || '').trim()).toUpperCase()); return badges.includes('DISPLAY') && badges.includes('CTV'); }); })()",
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-authored-table-badge-save-reopen.png",
      fullPage: true,
    },
  ],
};
