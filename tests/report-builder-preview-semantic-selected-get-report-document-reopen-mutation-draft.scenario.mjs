...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForDomContains",
      text: "Reopened ReportDocument: Report Builder Demo",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const root = Array.from(document.querySelectorAll('.forge-report-builder__runtime-preview')).find((node) => node && node.offsetParent !== null); if (!root) { return false; } const titles = Array.from(root.querySelectorAll('section h3')).map((node) => (node.innerText || node.textContent || '').trim()).filter(Boolean); const bodyMatches = ((root.innerText || root.textContent || '').match(/Author-provided inventory context\\./g) || []).length; return titles.includes('Inventory Note') && titles.includes('Inventory Note Copy') && bodyMatches >= 2; })()",
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-selected-get-report-document-reopen-mutation-draft.png",
      fullPage: true,
    },
  ],
};
