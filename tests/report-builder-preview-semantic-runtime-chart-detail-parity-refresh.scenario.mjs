...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForEval",
      expression: "(() => { const intents = Array.from(document.querySelectorAll('.forge-report-runtime-host-intent')); const hasPreviewIntent = intents.some((entry) => entry.closest('.forge-report-builder__runtime-preview') && (entry.innerText || entry.textContent || '').includes('target://example/performance/channel-detail') && (entry.innerText || entry.textContent || '').includes('Display')); const hasStandaloneIntent = intents.some((entry) => !entry.closest('.forge-report-builder__runtime-preview') && (entry.innerText || entry.textContent || '').includes('target://example/performance/channel-detail') && (entry.innerText || entry.textContent || '').includes('Display')); return hasPreviewIntent && hasStandaloneIntent; })()",
      timeoutMs: 60000,
    },
    {
      type: "clickSelector",
    ...
    {
      type: "waitForEval",
      expression: "document.querySelectorAll('.forge-report-runtime-host-intent').length === 0",
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-runtime-chart-detail-parity-refresh.png",
      fullPage: true,
    },
  ],
};
