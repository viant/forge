...
export default {
  ...
  steps: [
    ...buildInlineArtifactImportSteps({
      ...
      filename: "capacity-trend.saved-view.history-card.saved-record.json",
    ...
    }),
    {
      ...
      expression: `(() => {
        ...
        const file = new File([JSON.stringify(artifact, null, 2)], 'capacity-trend.published-snapshot.history-card.saved-record.json', { type: 'application/json' });
    ...
    ...buildInlineArtifactImportSteps({
      ...
      filename: "capacity-trend.saved-view.history-card.list-response.json",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; return text.includes('Recent exports') && text.includes('Capacity Trend Q3 Saved View') && text.includes('Refresh status'); })()",
      timeoutMs: 60000,
    },
    buildTitledCardButtonClickStep({
      title: "Capacity Trend Q3 Saved View",
      buttonTexts: ["Refresh status"],
      missingButtonMessage: "Selected saved-view recent export refresh button not found.",
    }),
    {
      type: "waitForDomContains",
    ...
    buildTitledCardButtonClickStep({
      ...
      missingButtonMessage: "Selected saved-view recent export download button not found.",
    }),
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "window.__artifactDownloadCapture && window.__artifactDownloadCapture.mimeType === 'application/pdf' && window.__artifactDownloadCapture.payload.includes('%PDF-selected-entry saved-view history saved_view_capacityTrendQ3')",
      timeoutMs: 60000,
    },
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-list-entry-saved-view-history-card-actions.png",
      fullPage: true,
    },
  ],
};
