...
export default {
  ...
  steps: [
    ...
    {
      type: "eval",
      expression: "(() => { const preview = window.__REPORT_BUILDER_PREVIEW__; if (!preview || typeof preview.applyLocalTableCalculationDraft !== 'function') { throw new Error('applyLocalTableCalculationDraft API not available.'); } const result = preview.applyLocalTableCalculationDraft({ draft: { id: 'runningCtvAvails', label: 'Running CTV Avails', functionId: 'runningTotal', sourceField: 'ctvAvails', orderByField: 'eventDate', orderDir: 'asc', partitionBy: ['channelV2'] } }); return !!result?.valid && result?.field?.id === 'runningCtvAvails' && result?.prepared?.canApply === true; })()",
    },
    {
      type: "waitForEval",
      expression: "(() => { const keys = ['reportBuilder.state.demoReportBuilder.demoReportBuilderWindow', 'reportBuilder.state.demoReportBuilder']; return keys.some((key) => { const raw = window.localStorage.getItem(key); if (!raw) { return false; } try { const parsed = JSON.parse(raw); return Array.isArray(parsed?.localTableCalculations) && parsed.localTableCalculations.some((entry) => entry?.id === 'runningCtvAvails' && entry?.compute?.type === 'runningTotal' && entry?.compute?.sourceField === 'ctvAvails'); } catch (_) { return false; } }); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const headers = Array.from(document.querySelectorAll('.forge-report-builder__table th')).map((entry) => (entry.innerText || entry.textContent || '').trim().toLowerCase()); return headers.includes('running ctv avails') && headers.includes('ctv avails') && headers.includes('channel') && headers.includes('delivery date'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
      expression: "(() => { const text = document.querySelector('.forge-report-runtime-table-panel')?.innerText || document.querySelector('.forge-report-runtime-table-panel')?.textContent || ''; return text.includes('Running CTV Avails') && text.includes('70,300') && text.includes('CTV Avails') && text.includes('34,300'); })()",
      timeoutMs: 60000,
    },
    {
      type: "reload",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const state = window.__REPORT_BUILDER_PREVIEW__?.getBuilderState?.(); return Array.isArray(state?.localTableCalculations) && state.localTableCalculations.some((entry) => entry?.id === 'runningCtvAvails' && entry?.compute?.type === 'runningTotal' && entry?.compute?.sourceField === 'ctvAvails'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const headers = Array.from(document.querySelectorAll('.forge-report-builder__table th')).map((entry) => (entry.innerText || entry.textContent || '').trim().toLowerCase()); return headers.includes('running ctv avails') && headers.includes('ctv avails'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForDomContains",
      text: "\"localTableCalculations\": [",
      timeoutMs: 60000,
    },
    {
      type: "waitForDomContains",
      text: "\"id\": \"runningCtvAvails\"",
      timeoutMs: 60000,
    },
    {
      type: "waitForDomContains",
      text: "\"sourceField\": \"ctvAvails\"",
      timeoutMs: 60000,
    },
    {
      type: "fillSelector",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const panel = document.querySelector('[aria-label=\"Get ReportDocument response summary\"]'); const text = panel?.innerText || panel?.textContent || ''; return text.includes('\"kind\": \"getReportDocumentResponse\"') && text.includes('\"id\": \"runningCtvAvails\"') && text.includes('\"sourceField\": \"ctvAvails\"') && text.includes('\"id\": \"ctvAvails\"'); })()",
      timeoutMs: 60000,
    },
    ...buildReopenedHydratedSessionVerificationSteps({
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-table-calc-authoring.png",
      fullPage: true,
    },
  ],
};
