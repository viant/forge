...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; return text.includes('demo-export-artifact-1') && text.includes('succeeded') && text.includes('Semantic Binding') && text.includes('Model Ad Delivery') && text.includes('Entity Line Delivery') && text.includes('Available Impressions') && text.includes('PREVIEW FILL'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-import-report-fill-detach-report-spec-export-artifact-failure.png",
      fullPage: true,
    },
  ],
};
