...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForDomContains",
      text: "Could not load the preview export artifact for Capacity Trend Q3 Published Snapshot.",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; return text.includes('demo-export-artifact-1') && text.includes('succeeded') && text.includes('publishedSnapshot') && text.includes('reportBuilder.publishedSnapshot://published_snapshot_capacityTrendQ3') && text.includes('Semantic Binding') && text.includes('Model Ad Delivery') && text.includes('Entity Line Delivery') && text.includes('Available Impressions') && text.includes('Household Uniques'); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-import-published-snapshot-export-artifact-failure.png",
      fullPage: true,
    },
  ],
};
