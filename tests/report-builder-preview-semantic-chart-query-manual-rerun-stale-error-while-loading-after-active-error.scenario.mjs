...
export default {
  ...
  steps: [
    ...
    {
      type: "waitForEval",
      expression: "(() => { const preview = window.__REPORT_BUILDER_PREVIEW__; return !!preview && Array.isArray(preview.fetchBehaviors) && preview.fetchBehaviors.length === 2 && preview.fetchBehaviors[0]?.match?.type === 'chartquery' && preview.fetchBehaviors[0]?.delayMs === 1200 && preview.fetchBehaviors[0]?.error === 'Stale chart query failed.' && preview.fetchBehaviors[1]?.match?.type === 'chartquery' && preview.fetchBehaviors[1]?.delayMs === 5000 && preview.fetchBehaviors[1]?.error === 'Active chart query failed.'; })()",
      timeoutMs: 60000,
    },
    buildPreviewPatchBuilderStateStep({
      patch: {
    ...
    {
      type: "eval",
      expression: "(() => { const button = document.querySelector('.forge-report-builder__run-button'); if (!button) { throw new Error('run button not found'); } if (button.disabled) { throw new Error('run button disabled during manual rerun dual error setup'); } button.click(); return true; })()",
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; const preview = window.__REPORT_BUILDER_PREVIEW__; const current = { errorCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'error').length, successCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'success').length }; const settled = preview.__chartManualRerunDualErrorPreActiveCounts; const changed = !settled || settled.errorCount !== current.errorCount || settled.successCount !== current.successCount; if (changed) { preview.__chartManualRerunDualErrorPreActiveCounts = { ...current, lastChangedAt: Date.now() }; return false; } return current.errorCount === 1 && current.successCount === 0 && Date.now() - settled.lastChangedAt >= 350 && !text.includes('Stale chart query failed.') && !text.includes('Active chart query failed.') && !text.includes(\"We couldn't render these results\"); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "waitForEval",
      expression: "(() => { const text = document.body?.innerText || document.body?.textContent || ''; const preview = window.__REPORT_BUILDER_PREVIEW__; const current = { errorCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'error').length, successCount: (preview.fetchEventHistory || []).filter((entry) => entry.type === 'chartQuery' && entry.phase === 'success').length }; const settled = preview.__chartManualRerunDualErrorFinalCounts; const changed = !settled || settled.errorCount !== current.errorCount || settled.successCount !== current.successCount; if (changed) { preview.__chartManualRerunDualErrorFinalCounts = { ...current, lastChangedAt: Date.now() }; return false; } return current.errorCount === 2 && current.successCount === 0 && Date.now() - settled.lastChangedAt >= 1200 && text.includes('Active chart query failed.') && !text.includes('Stale chart query failed.') && text.includes(\"We couldn't render these results\"); })()",
      timeoutMs: 60000,
    },
    {
      type: "waitForEval",
    ...
    {
      type: "screenshot",
      file: "report-builder-preview-semantic-chart-query-manual-rerun-stale-error-while-loading-after-active-error.png",
      fullPage: true,
    },
  ],
};
