...
data class UIActionEffect(val kind: String, val ref: String?, val value: JsonElement)
data class UIActionProjectionResult(val effects: List<UIActionEffect>, val result: JsonElement?)

/** Snapshot-only metadata UI execution. The wrapper can describe effects but cannot access host APIs. */
object UIActionProjection {
    suspend fun invoke(
        code: String,
        functionName: String,
        namespace: String?,
        ...
        windowForm: JsonObject,
        props: JsonObject
    ): UIActionProjectionResult {
        ...
        val wrapper = """
            (() => {
              ...
              const setTimeout = (fn, delay = 0) => {
                if (delay !== 0) throw new Error('Delayed UI action timers are unsupported');
            ...
            $code
              ...
              return { run: ({name, source, snapshots, windowForm, props}) => {
                ...
                const signal = (state, key, kind, ref) => {
                  const holder = {peek: () => state[key]};
                  Object.defineProperty(holder, 'value', {get: () => state[key], set: value => {state[key]=value;effects.push({kind,ref,value});}});
                  return holder;
                };
                ...
                const contextFor = ref => {
                  ...
                  const ctx = {
                    identity:{dataSourceRef:ref}, Context:contextFor,
                    signals:{
                      form:signal(state,'form','form',ref), collection:{peek:()=>state.collection||[],get value(){return state.collection||[];}},
                      selection:{peek:()=>state.selection||{},get value(){return state.selection||{};}},
                      input:{peek:()=>state.input||{},get value(){return state.input||{};}},
                    ...
                    handlers:{
                      dataSource:{
                        ...
                        setInputParameters:value=>{state.input=value||{};effects.push({kind:'input',ref,value:state.input});return true;},
                        setFilter:value=>{effects.push({kind:'filter',ref,value:value||{}});return true;},
                        fetchCollection:options=>{effects.push({kind:'fetch',ref,value:options||{}});return true;},
                        resetSelection:()=>{effects.push({kind:'resetSelection',ref,value:{}});return true;}
                      },
                      window:{
                        openDialog:value=>{effects.push({kind:'openDialog',ref:null,value:value||{}});return true;},
                        closeDialog:value=>{effects.push({kind:'closeDialog',ref:null,value:value||{}});return true;},
                        openWindow:value=>{effects.push({kind:'openWindow',ref:null,value:value||{}});return true;},
                        openTarget:value=>{effects.push({kind:'openTarget',ref:null,value:value||{}});return true;}
                      }
                    }
                  };
                ...
                const result=fn({...props,context:contextFor(source)});
                if(result&&typeof result.then==='function')throw new Error('Async UI actions are unsupported');
        ...
        """.trimIndent()
        val envelope = ActionHookRuntime.invoke(
            wrapper,
            "run",
            JsonObject(mapOf("name" to JsonPrimitive(name), "source" to JsonPrimitive(source), "snapshots" to snapshots, "windowForm" to windowForm, "props" to props))
        ) as? JsonObject ?: return UIActionProjectionResult(emptyList(), null)
        val effects = (envelope["effects"] as? JsonArray).orEmpty().mapNotNull { raw ->
            val obj = raw as? JsonObject ?: return@mapNotNull null
            val kind = (obj["kind"] as? JsonPrimitive)?.contentOrNull ?: return@mapNotNull null
            UIActionEffect(kind, (obj["ref"] as? JsonPrimitive)?.contentOrNull, obj["value"] ?: JsonNull)
        }
        return UIActionProjectionResult(effects, envelope["result"]?.takeUnless { it is JsonNull })
    }
}
