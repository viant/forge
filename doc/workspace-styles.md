...
`typography.family: workspace-primary` selects the active workspace's primary
typeface without encoding a vendor or font classification in Forge. The
workspace style service validates and registers that family once; Forge
surfaces and native form controls inherit it from the theme boundary. Code and
terminal roles retain their explicit monospace stacks.

Standalone dashboard HTML exports use `--forge-font-family`. Downloaded HTML
does not copy authenticated workspace font files or remote URLs, so it
intentionally falls back to the local system family unless the caller supplies
a licensed standalone stylesheet. This keeps offline exports functional
without silently embedding font assets.
